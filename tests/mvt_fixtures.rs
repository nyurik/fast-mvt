...
use fast_mvt::proto::{GeomType, Tile};
use fast_mvt::{DEFAULT_EXTENT, MvtReaderRef, MvtResult};
use serde::Deserialize;
use serde_json::Value;
use test_each_file::test_each_path;

test_each_path! { for ["mvt"] in "fixtures/mvt-fixtures/fixtures" => mvt_reader_fixture }
...
fn normalize_default_geometry_types(tile: &mut Tile) {
    for layer in &mut tile.layers {
        for feature in &mut layer.features {
            if feature.r#type == Some(GeomType::UNKNOWN) {
                feature.r#type = None;
            }
        }
    }
